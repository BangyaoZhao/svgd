...
evaluation <- function(svgd_bnn, X_test, y_test) {
  ...
  X_test <- t(apply(X_test, 1, function(x) {(x-mean_X_train)/sd_X_train}))

  d <- ncol(X_test)

  pred_y_test <- array(0, dim = c(M, output_dim, dim(y_test)[2]))
  prob <- matrix(rep(0, times = M * length(y_test)), nrow = M, ncol = dim(X_test)[1])

  for (i in 1:M) {
    para_list <- unpack_parameters(theta[i, ], d, num_nodes)
    loggamma <- para_list$loggamma
    pred_y_test[i, , ] <- forward_probagation(t(X_test), para_list, 'relu')$ZL * sd_y_train + mean_y_train
    prob[i, ] <- (exp(loggamma))^(output_dim/2) / ((2 * pi)^(output_dim/2)) *
  ...
  return(list(svgd_rmse = svgd_rmse, svgd_Rsq = svgd_Rsq, svgd_ll = svgd_ll))
}
